...
function showPlayers(players) {
  // index++;
  // console.log(index);
  // document.querySelector('card-photo').dataset.index = index;  
  return `
    <div id="card-photo" class="relative w-[250px] h-[400px] bg-cover" style="background-image: url('/assets/images/card.png');">
     <div onclick="deletePlayer(${index})" class="top-8 left-8 relative"> 
...
function showAllPlayers (){
    ...
    players.forEach((element, index) => {
        ...
        div.innerHTML= showPlayers(element,index)
...
//     let play = JSON.parse(localStorage.players)
...
function deletePlayer(index) {
  ...
  const container = document.getElementById('cardContainer'); // Ensure this ID matches your container
  container.innerHTML = ''; // Clear existing players
  players.forEach((player, newIndex) => {
    const cardHTML = showPlayer(player, newIndex);
    container.innerHTML += cardHTML; // Append the updated cards
  });
}
